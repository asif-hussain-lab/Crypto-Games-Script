...
if (isset($_POST["bsave"])) {
  ...
  $reason = $_POST["breason"];
  $ip = $_POST["bip"];

 $addtodbbanneduser = AddBannedUserToDB($username, $reason, $ip);

 if ($addtodbbanneduser == 'ok') {
   $message = '<div class="ui positive message">
  <div class="header">
    You Succesfully banned '.$username.'
  </div>
</div>';
 } else {
   echo "internal error";
...
//Unban User
 if (isset($_POST["unbsave"])) {

   $id = $_POST["unbid"];

 $unbanuser = UnbanUser($id);

 if ($unbanuser == 'ok') {
   $message = '<div class="ui positive message">
  <div class="header">
    You Succesfully unbanned user with  '.$id.' ID.
  </div>
</div>';
 } else {
   echo "internal error";
...
<nav class="navbar navbar-expand-lg navbar navbar-dark" style="background-color: #05437b;">
 ...
 <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
          ...
          <a class="dropdown-item active" href="ban"><i class="user times icon"></i>Ban Users</a>
...
<br>
<br>
<center><h3>Total Banned Users = <a class="ui teal big label"><?php echo $lastid; ?></h3></a></center>
<br>
<br>
<!-- Button trigger modal -->
<center><button type="button" class="ui red button" data-toggle="modal" data-target="#banModal">
 <i class="user times icon"></i> Ban User
</button>
<button type="button" class="ui green button" data-toggle="modal" data-target="#unbanModal">
 <i class="user plus icon"></i> Unban User
</button>
</center>
<!-- BAN Modal -->
...
<div class="ui large left icon input">
  <input placeholder="Username of user you want to ban" name="busername" type="text">
  <i class="user blue icon"></i>
</div>
<br>
  <h5>Reason :</h5>
<div class="ui large left icon input">
  <input placeholder="Reason of Bann" name="breason" type="text">
  <i class="comment blue icon"></i>
</div>
  <h5>User IP :</h5>
<div class="ui large left icon input">
  <input placeholder="IP of user" name="bip" type="text">
  <i class="i cursor blue icon"></i>
      ...
      <div class="modal-footer">
        ...
        <button name="bsave" class="ui blue button"><i class="user times icon"></i>Ban User</button>
        </form>
      </div>
    </div>
  </div>
...
<div class="modal fade" id="unbanModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Unban Users</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <center>
          <form method="post" action="">
  <h5>ID Of Banned User :</h5>
<div class="ui large left icon input">
  <input placeholder="ID Of Banned User" name="unbid" type="text">
  <i class="user blue icon"></i>
</div>
<br>
</center>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <button name="unbsave" class="ui green button"><i class="user times icon"></i> Unban User</button>
        </form>
      </div>
    </div>
  </div>
