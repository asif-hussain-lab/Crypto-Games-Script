...
if(isset($_POST["query"]))
{
 ...
 $query = "
  ...
  OR type LIKE '%".$search."%' OR status LIKE '%".$search."%' OR amount LIKE '%".$search."%' ORDER BY created_at DESC
 ";
}
else
{
 $query = "
  SELECT * FROM withdraw_transactions ORDER BY created_at DESC
 ";
}
...
if ($row["status"] == 'Paid') {
  ...
  $status = '<a class="ui red label">Refunded</a>';
}
