...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
      ...
      $coinhive_api = $row["coinhive_api"]; //NEW
      ...
      //Minimum amount to bet and play LuckyNumber
      $min_amount_luckynumber = $row["min_amount_luckynumber"];

      //Mining reward for 100,000 hashes/solved //NEW
      $satreward = number_decimal($row["satreward"]);

      //Minimum withdrawal amount for mining service
